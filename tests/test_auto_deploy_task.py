...
async def test_wait_for_dependencies(
    loop, valid_portainer_config, mocker, fake_app_session
):
    mock = mocker.patch(
    ...
    await auto_deploy_task.wait_for_dependencies(
        valid_portainer_config, fake_app_session
    )
    ...
    for portainer_cfg in valid_portainer_config["main"]["portainer"]:
        calls.append(
            mocker.call(
                URL(portainer_cfg["url"]),
                fake_app_session,
                portainer_cfg["username"],
                portainer_cfg["password"],
            )
        )
...
async def test_setup_task(loop, fake_app, mocker):
    ...
    mock_git_changes = mocker.patch.object(
        auto_deploy_task.GitUrlWatcher, "check_for_changes", return_value=Future()
    )
