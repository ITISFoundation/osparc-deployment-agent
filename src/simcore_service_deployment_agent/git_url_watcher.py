...
from .subprocess_utils import exec_command
...
async def _git_get_FETCH_HEAD_sha(directory: str) -> str:
    ...
    sha = await exec_command(cmd, f"{directory}")
    return sha


async def _git_get_sha_of_tag(directory: str, tag: str) -> str:
    cmd = ["git", "rev-list", "-1", "--sparse", tag]
    sha_long = await exec_command(cmd, f"{directory}")
    cmd = ["git", "rev-parse", "--short", sha_long]
    sha_short = await exec_command(cmd, f"{directory}")
    return sha_short
...
async def _git_get_tag_created_dt(directory: str, tag: str) -> Optional[datetime]:
    ...
    """
    date_string = await exec_command(
...
async def _git_clean_repo(directory: str):
    cmd = ["git", "clean", "-dxf"]
    await exec_command(cmd, f"{directory}")
...
async def _git_get_latest_matching_tag(
    ...
    repo_tags_msg = await exec_command(
        [
            "git",
            "tag",
            "--list",
...
async def _git_get_current_matching_tag(repo: GitRepo) -> list[str]:
    ...
    all_tags_str = await exec_command(
        [
            "git",
            "show-ref",
            "--tags",
    ...
    sha_to_be_found = await exec_command(cmd2, f"{repo.directory}")
...
async def _git_diff_filenames(
    ...
    modified_files = await exec_command(cmd, f"{directory}")
...
async def _git_get_logs(
    ...
    logs = await exec_command(cmd, f"{directory}", strip_endline=False)
    return logs
...
async def _git_get_logs_tags(
    ...
    logs = await exec_command(cmd, f"{directory}")
    return logs
...
async def _checkout_repository(repo: GitRepo, tag: Optional[str] = None):
    ...
    cmd = ["find", "."]
    files_in_repo = (await exec_command(cmd, f"{repo.directory}")).split("\n")
...
async def _check_if_tag_on_branch(repo_path: str, branch: str, tag: str) -> bool:
    ...
    try:
        data = await exec_command(cmd, repo_path)
