...
async def notify_mattermost(
    mattermost_config: Dict, app_session: ClientSession, add_message: str
):
    if mattermost_config["enabled"]:
        ...
        async with app_session.post(
            url, headers=headers, json={"channel_id": channel_id, "message": message}
        ) as resp:
...
async def notify_mattermost_header(
    ...
    if mattermost_config["enabled"]:
        ...
        async with app_session.get(url, headers=headers) as resp:
            log.debug(
                "requested channel description: received with code %s", resp.status
            )
            if resp.status == 404:
                log.error("could not find route in %s", url)
                raise ConfigurationError(
        ...
        if start_index != -1:
            ...
            new_header = "{}{}{}".format(
                current_header[0:start_index], message, current_header[lastindex + 1 :]
            )

        url = URL(mattermost_config["url"]).with_path(
            f"api/v4/channels/{channel_id}/patch"
        )
        async with app_session.put(
            url, headers=headers, data=json.dumps({"header": new_header})
        ) as resp:
            log.debug(
...
async def notify(app_config: Dict, app_session: ClientSession, message: str = None):
    if "notifications" in app_config["main"]:
...
async def notify_state(
    app_config: Dict, app_session: ClientSession, state: State, message: str
):
    if "notifications" in app_config["main"]:
        notify_configs = app_config["main"]["notifications"]
        for notify_config in notify_configs:
            if "mattermost" == notify_config["service"]:
                await notify_mattermost_header(
                    notify_config, app_session, state, message
                )
